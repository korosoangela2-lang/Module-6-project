...
export const Button = ({
  ...
  const styles = {
    primary: {
      background: `linear-gradient(180deg, ${T.pine}, ${T.pine})`,
      color: "#fff",
      border: `1px solid ${T.pine}`,
      boxShadow: "0 6px 18px rgba(10,95,79,0.12)",
    },
    dark: {
      background: `linear-gradient(180deg, ${T.ink}, ${T.ink})`,
      color: "#fff",
  ...
  return (
    ...
    >
      {loading ? (
        <Loader2 size={15} className="heha-spin" />
      ) : Icon ? (
        <Icon size={15} />
      ) : null}
      {children}
...
export const TextInput = ({ style, className, ...rest }) => (
  <input
    ...
    className={`heha-input ${className || ""}`}
  />
);
...
export const StatusPill = ({ status }) => (
  <Pill tone={STATUS_TONE[status] || "muted"}>{status}</Pill>
);
    ...
    >
      <div
        ...
        style={{
          width: 34,
          height: 34,
          background: out ? T.brickSoft : T.pineSoft,
          color: out ? T.brick : T.pine,
          flexShrink: 0,
        }}
      >
